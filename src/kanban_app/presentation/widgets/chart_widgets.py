...
class PunctualityDonutWidget(QWidget):
    """Gráfico de anel (Donut) com taxa de pontualidade das entregas."""
    ...
    def paintEvent(self, event) -> None:
        ...
        h = self.height()
        side = min(w, h - 35)
        rect = QRectF((w - side) / 2 + 10, 10, side - 20, side - 20)

        pen_width = max(12, int(side * 0.14))
        rect.adjust(pen_width / 2, pen_width / 2, -pen_width / 2, -pen_width / 2)

        total = self._on_time_count + self._delayed_count
        if total == 0:
            # Sem dados: anel cinza
            gray_pen = QPen(QColor("#cbd5e1"), pen_width, Qt.PenStyle.SolidLine)
            ...
            # Fundo vermelho para atrasos (ou anel base)
            ...
            green_pen = QPen(QColor("#16a34a"), pen_width, Qt.PenStyle.SolidLine)
            painter.setPen(green_pen)
            painter.drawArc(rect, 90 * 16, -span_angle)

            # Texto central
            painter.setPen(QColor("#0f172a"))
            f_num = QFont("Segoe UI", max(12, int(side * 0.16)), QFont.Weight.Bold)
            ...
            painter.setPen(QColor("#64748b"))
            lbl_rect = QRectF(rect.x(), rect.y() + rect.height() * 0.45, rect.width(), rect.height() * 0.35)
            painter.drawText(lbl_rect, Qt.AlignmentFlag.AlignCenter, "No Prazo")

        # Legenda inferior
        leg_rect = QRectF(0, h - 28, w, 24)
...
class SectorBarChartWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        for idx, sec in enumerate(displayed):
            ...
            text_rect = QRectF(10, y, label_w - 10, row_h)
            ...
            bar_color = QColor(sec.cor if sec.cor else "#475569")
            painter.setBrush(QBrush(bar_color))
            painter.drawRoundedRect(QRectF(bar_x, bar_y, fill_w, 14), 4, 4)

            # Quantidade e Peças à direita
            painter.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
            painter.setPen(QColor("#475569"))
            val_rect = QRectF(bar_x + bar_area_w + 6, y, val_w, row_h)
